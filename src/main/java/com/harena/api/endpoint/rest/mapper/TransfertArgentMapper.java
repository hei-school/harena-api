...
class TransfertArgentMapper implements Mapper<TransfertArgent, com.harena.api.endpoint.rest.model.TransfertArgent> {
  ...
  @Override
  public com.harena.api.endpoint.rest.model.TransfertArgent toRestModel(TransfertArgent objectModel) {
    // The worst things ! cause field: transfertCommeGroupe don't have getter
    GroupePossession groupePossession = (GroupePossession) getPrivateFieldValue("transfertCommeGroupe", objectModel);
    // The worst things ! cause field: possessions don't have getter
    Set<Possession> possessions = (Set<Possession>) getPrivateFieldValue("possessions", groupePossession);
    com.harena.api.endpoint.rest.model.GroupePossession restGroupePossession = new com.harena.api.endpoint.rest.model.GroupePossession();
    ...
    restGroupePossession.setPossessions(possessions.stream().map(possesionMapper::toRestModel).toList());
  ...
  public TransfertArgent toObjectModel(com.harena.api.endpoint.rest.model.TransfertArgent restModel) {
    ...
    LocalDate debut = restModel.getTransfertCommeGroupe().getPossessions().getFirst().getFluxArgent().getDebut();
    LocalDate fin = restModel.getTransfertCommeGroupe().getPossessions().getFirst().getFluxArgent().getFin();
    var fluxMensuel = restModel.getTransfertCommeGroupe().getPossessions().getFirst().getFluxArgent().getFluxMensuel();
    var dateOperation = restModel.getTransfertCommeGroupe().getPossessions().getFirst().getFluxArgent().getDateDOperation();
    if (restDevise == null) {
      return new TransfertArgent(nom, depuisArgent, versArgent,debut, fin,fluxMensuel,dateOperation);
    }
