...
class AchatMaterielAuComptantMapper implements Mapper<AchatMaterielAuComptant, com.harena.api.endpoint.rest.model.AchatMaterielAuComptant> {
  ...
  public com.harena.api.endpoint.rest.model.AchatMaterielAuComptant toRestModel(AchatMaterielAuComptant objectModel) {
    ...
    GroupePossession groupePossession = (GroupePossession) getPrivateFieldValue("achatCommeGroupe", objectModel);
  ...
  public AchatMaterielAuComptant toObjectModel(com.harena.api.endpoint.rest.model.AchatMaterielAuComptant restModel) {
    ...
    Double tauxAppreciationAnnuelle = Objects.requireNonNull(Objects.requireNonNull(Objects.requireNonNull(restModel.getAchatCommeGroupe()).getPossessions()).getFirst().getMateriel()).getTauxDappreciationAnnuel();
    Argent financeur = argentMapper.toObjectModel(restModel.getAchatCommeGroupe().getPossessions().getLast().getFluxArgent().getArgent());
    if (restDevise == null) {
      return new AchatMaterielAuComptant(nom, tTime, valeurComptableALAchat, tauxAppreciationAnnuelle, financeur);
    }
